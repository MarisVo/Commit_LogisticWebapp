...
warehouseAdminRoute.post("/", async (req, res) => {
  ...
  try {
    ...
    await opencage
      .geocode({
        q: `${street},${ward},${district},${province}`,
 ...
 * @access private
 */
warehouseAdminRoute.put("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { name, phone, street, ward, district, province } = req.body;
    ...
    if (warehouse) {
      ...
      if (street && ward && district && province) {
        ...
        if (lon || lat) {
          ...
          return sendSuccess(res, "Update warehouse successfully", {
            name,
            phone,
            street,
            ward,
 ...
 * @access private
 */
warehouseAdminRoute.delete("/:id", async (req, res) => {
  const { id } = req.params;
  try {
