 ...
 async (req, res) => {
    ...
    if (errors)
     ...
     try {
         ...
         if (isExist)
            return sendError(res, 'Department already exists')

        await Department.create({
            name, description, location, director, scale
         })
     } catch (error) {
...
departmentAdminRoute.put('/:id',
    async (req, res) => {
        ...
        try {
            ...
            if (department){
                await department.findByIdAndUpdate(id, { name: name, description: description, location: location, director: director, scale: scale })
                return sendSuccess(res, "Update department successfully.", { name: name, description: description, location: location, director: director, scale: scale })
            }        
 ...
 * @access private
 */
departmentAdminRoute.delete('/:id',
    async (req, res) => {
        const {id} = req.params    
        try {
            ...
            await Department.findByIdAndRemove(id)
                .then(()=> { return sendSuccess(res, "Delete department successfully.")})  
