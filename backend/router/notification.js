...
notificationRoute.post('/', async (req, res) => {
    ...
    try {
        ...
        if (existReceiver) {
            await Notification.create({ sender, receiver: receiverId, title, message, link })
...
notificationRoute.get('/', async (req, res) => {
    ...
    try {
        const notifications = await Notification.find({ receiver: userId }).sort({ 'updatedAt': -1 }).limit(limit)
